...
public class CAPIGenerator {
	...
	public void generateClassAPI(PrintWriter hOut, PrintWriter cOut,
		...
		for (int i = 0; i < constructors.length; i++) {
			...
			if (Modifier.isPublic(constructor.getModifiers())) {
				...
				constructor2name.put(constructor, getMethodName(constructor));
			}
		}
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if (Modifier.isPublic(mod)) {
				...
				method2name.put(method, getMethodName(method));
			}
		}
	...
	public static void main(String[] args) throws ClassNotFoundException {
		if (args.length < 3) {
			System.err.println("Usage: " + CAPIGenerator.class.getName()
					+ " file.h file.c common_include.h <java class files>");
			System.exit(-1);
		}
		...
		try {
			...
			for(int i = 3; i < args.length; i++) {
				...
				new CAPIGenerator(clz).generateClassAPI(hOut, cOut,
						false, false);
			}
	...
	private StringBuffer appendJVMConstructorSignature(Constructor constructor,
			StringBuffer buff) {
		buff.append("(");
	...
	private String getMethodName(Method method) {
		...
		if(methodName.equals("array")) { 
			...
			methodName = "array"+getJVMTypeSignature(returnType);
		}
		if (argumentTypes == null || argumentTypes.length == 0)
			return mangle(methodName);
		else
			return mangle(methodName + "__"
					+ getJVMTypeSignatures(argumentTypes));
	}
	
	private String getMethodName(Constructor ctr) {
		Class[] argumentTypes = ctr.getParameterTypes();
		if (argumentTypes == null || argumentTypes.length == 0)
			return mangle("new");
		else
			return mangle("new" + "__"
					+ getJVMTypeSignatures(argumentTypes));
	}
}
